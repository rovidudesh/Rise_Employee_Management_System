...
const AssignedTasksQuickView = () => {
  ...
  const dummyTasks: AssignedTask[] = [
    ...
    },
    {
      ...
      description: "Update API documentation and add comments to core functions",
      priority: "medium",
      status: "pending",
    ...
    },
    {
      ...
      description: "Optimize queries and improve database response time",
      priority: "medium",
      status: "completed",
      assigned_date: "2025-01-01",
      due_date: "2025-01-08",
      assigned_by: "Mike Senior Dev"
    },
    {
      id: 5,
  ...
  useEffect(() => {
    ...
    const fetchTasks = async () => {
      ...
      // Filter for pending and in-progress tasks (most relevant)
      const activeTasks = dummyTasks.filter(task => 
  ...
  const handleTaskClick = (task: AssignedTask) => {
    ...
    const dueDateInfo = daysUntilDue > 0 ? `Due in ${daysUntilDue} days` : 
    ...
    const details = [
      ...
      `Due: ${formatDate(task.due_date)} (${dueDateInfo})`
    ].join('\n');
    ...
    <div className="w-full max-w-lg bg-white rounded-lg shadow-md p-6 h-fit">
          ...
          return (
            ...
            >
              <div className="flex items-center justify-between mb-2">
                ...
                <span className={`px-2 py-1 inline-flex text-xs leading-4 font-medium rounded-full ${getPriorityClasses(task.priority)} flex-shrink-0`}>
                  {task.priority}
                </span>
              </div>
              <div className="mb-2">
              ...
              <div className="flex items-center justify-between mb-2">
                <span className={`px-2 py-1 inline-flex text-xs leading-4 font-medium rounded-full ${getStatusClasses(task.status)} flex-shrink-0`}>
      ...
      <div className="mt-4 pt-3 border-t border-gray-200">
        <div className="flex items-center justify-between text-xs text-gray-500">
          <span>🔥 Urgent: {tasks.filter(task => task.priority === 'urgent').length}</span>
          <span>⚠️ Overdue: {tasks.filter(task => task.status === 'overdue').length}</span>
        </div>
      </div>
    </div>
  );
