...
const TeamQuickView = () => {
  ...
  const showCustomMessage = (message: string) => {
    ...
    messageBox.innerHTML = `
      <div class="bg-white dark:bg-slate-800 p-6 rounded-lg shadow-xl text-center max-w-sm mx-auto border border-gray-200 dark:border-gray-700">
        <p class="text-sm font-semibold text-gray-800 dark:text-gray-200 mb-4">${message}</p>
  ...
  if (loading) {
    return (
      <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 p-5 h-fit transition-colors duration-300">
    ...
    <div className="w-full max-w-md bg-white dark:bg-slate-800 rounded-lg shadow-md border border-gray-200 dark:border-gray-700 p-5 h-fit transition-colors duration-300">
      ...
      <div className="space-y-3 max-h-96 overflow-y-auto">
        {employees.map((employee) => (
          <div
            key={employee.id}
            className="bg-gray-50 dark:bg-slate-700 rounded-lg p-4 border border-gray-200 dark:border-gray-600 cursor-pointer hover:bg-gray-100 dark:hover:bg-slate-600 transition-colors duration-150"
            onClick={() => handleEmployeeClick(employee)}
          >
            <div className="flex items-center justify-between mb-2">
              <span className="text-sm font-semibold text-gray-900 dark:text-gray-100 truncate pr-2">
                {employee.full_name}
              </span>
              <span
            ...
            <div className="flex items-center justify-between mb-2">
              ...
              <span
                className={`px-2 py-1 inline-flex text-xs leading-4 font-medium rounded-full ${getRoleClasses(
                  employee.role
                )}`}
              >
                {employee.role}
